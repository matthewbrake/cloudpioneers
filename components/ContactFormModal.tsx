 ...
 * It handles its own internal state for form submission and success messages.
 *
...
const ContactFormModal: React.FC<ContactFormModalProps> = ({ isOpen, onClose }) => {
  ...
  // Effect to handle opening and closing the dialog element programmatically.
  useEffect(() => {
  ...
  return (
    <dialog
      ...
      className="bg-transparent p-0 m-0 w-full h-full max-w-none max-h-none"
    >
        <div className="fixed inset-0 bg-background/80 backdrop-blur-sm flex items-center justify-center p-4 animate-fade-in">
          <div className="bg-primary border border-text-secondary/20 rounded-2xl w-full max-w-2xl shadow-2xl shadow-accent/10 relative animate-slide-up">
            
            {/* Close Button */}
            <button
              onClick={onClose}
              className="absolute top-4 right-4 text-text-secondary hover:text-accent transition-colors rounded-full p-2"
